...
那么 
  ...
  b. 链接 .o 文件为 .so 文件的时候，加 `-g -fno-omit-frame-pointer -fsanitize=address -fsanitize-recover=address -lasan`
  ...
  c. 运行时加前缀 `LD_PRELOAD=/usr/lib/gcc/x86_64-linux-gnu/5/libasan.so` ，这个 so 的路劲要自己找。
	...
	出现异常时，看不到出现异常的代码文件和行号，则运行时，继续增加前缀 ASAN_SYMBOLIZER_PATH=/usr/bin/llvm-symbolizer-3.8 ASAN_OPTIONS=symbolize=1
